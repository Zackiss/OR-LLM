...
def generate_neighbor_order(block_table: list[Block]):
    # 随机选择两个块更换装箱顺序
    new_block_table = copy.deepcopy(block_table)

    idx = range(len(new_block_table))
    index_a, index_b = random.sample(idx, 2)

    new_block_table[index_a], new_block_table[index_b] = new_block_table[index_b], new_block_table[index_a]
...
# 模拟退火主逻辑
def basic_heuristic(search_params, problem: Problem, method="rotate"):
    # 设置模拟退火的参数
    max_iterations = 1000
    initial_temp = 100
    cooling_rate = 0.95
    ...
    box_list = problem.box_list
    new_box_list = None

    # 开始模拟退火迭代，尝试更多可能的 block_table 组合，更换装箱顺序
    for i in range(max_iterations):
        ...
        if i > 0:
            if method == "rotate":
                new_box_list = generate_neighbor_rotate(box_list, problem.num_list)
                new_block_table = gen_complex_block(problem.container, new_box_list, problem.num_list)
            else:
                new_block_table = generate_neighbor_order(block_table)
        else:
            new_block_table = block_table
        # 初始化排样状态
        ...
        while pack_state.space_stack.size() > 0:
            ...
            space = pack_state.space_stack.top()
            block_list = gen_block_list(space, pack_state.avail_list, new_block_table)
            # 在每个装载阶段一个剩余空间被装载，装载分为两种情况：有可行 block，无可行 block
            if block_list:
                # 在有可行 block 时，算法按照块选择算法选择可行 block，然后将未填充空间切割成新的剩余空间
                # 查找下一个近似最优块
                block = find_next_block(pack_state, block_list, new_block_table, search_params)
                # 弹出顶部剩余空间
                pack_state.space_stack.pop()
                # 更新可用物品数量
                pack_state.avail_list = (np.array(pack_state.avail_list) - np.array(block.require_list)).tolist()
        ...
        delta_fitness = fitness(pack_state, problem) - temp_fitness
        if delta_fitness > 0 or random.uniform(0, 1) < math.exp(delta_fitness / current_temp):
            if delta_fitness != 0:
                print(f"迭代：{i}, 适应率变动：{delta_fitness}")
            # 更新找到的最佳状态
            temp_fitness = fitness(pack_state, problem)
            block_table = new_block_table
            if new_box_list is not None:
    ...
    # 绘制排样结果图，将旋转后的箱子信息同步到问题类
    problem.box_list = box_list
...
def run_solution(box_list: list, container: list):
    ...
    search_params = dict()
    # 如果设置为 “order”，那么更换装箱顺序；如果设置为 “rotate”，那么转动箱体
    basic_heuristic(search_params, problem, method="rotate")


if __name__ == "__main__":
    CONTAINER = [0, 0, 0, 587, 233, 220]
    BOX_LIST = [[108, 76, 30, 40, 1], [110, 43, 25, 33, 2], [92, 81, 55, 39, 3]]
