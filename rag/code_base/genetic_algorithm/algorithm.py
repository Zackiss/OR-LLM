...
def gen_block_list(space: Space, avail, block_table):
    ...
    返回的 block_list 是按 block_table 排列的。

    :param space: 全部空间
    :param avail: 剩余可用箱体的数量
    :param block_table: 所有可能 block 的列表
    """
    block_list = []
...
def estimate(pack_state: PackingState, block_table, search_params):
    """
    该函数用于评价某个块，使用深度优先搜索：depth_first_search()，返回评估值。
...
def find_next_block(pack_state: PackingState, block_list, block_table, search_params):
    ...
    for block in block_list:
        ...
        # 评价值
        fitness_ = estimate(pack_state, block_table, search_params)
        # 移除刚才添加的块
...
def generate_neighbor_rotate(block_list: list[Box], num_list):
    new_box_list = []
    # 块的种类数
    type_num = len(num_list)
    selected_type = random.choice(range(type_num))
    # 旋转方式
    selected_way = random.choice([0, 1, 2])
    # 随机旋转一类块
    for box in block_list:
        if box.type == selected_type:
            if selected_way == 1:
                box.lx, box.ly, box.lz = box.ly, box.lz, box.lx  # 绕 x 轴旋转
            elif selected_way == 2:
                box.lx, box.ly, box.lz = box.lz, box.lx, box.ly  # 绕 y 轴旋转
            else:
                box.lx, box.ly, box.lz = box.ly, box.lz, box.lx  # 绕 z 轴旋转
        new_box_list.append(box)
    return new_box_list


# 创建初始群体的辅助函数
def initialize_population(box_list, num_individuals):
    population = []
    for _ in range(num_individuals):
        individual = generate_neighbor_rotate(box_list, [box.type for box in box_list])
...
# 随机变异函数
def mutate(individual, num_list):
    return generate_neighbor_rotate(individual, num_list)


# 适应度评价函数
def evaluate_fitness(box_list, problem):
    block_table = gen_complex_block(problem.container, box_list, problem.num_list)
    ...
    while pack_state.space_stack.size() > 0:
        ...
        if block_list:
            block = find_next_block(pack_state, block_list, block_table, {})
            pack_state.space_stack.pop()
            pack_state.avail_list = (np.array(pack_state.avail_list) - np.array(block.require_list)).tolist()
    ...
    return pack_state.volume / (problem.container.lx * problem.container.ly * problem.container.lz)


# 选择函数根据适应度选择最佳个体
def select(population, temp_fitness, num_to_select):
    selected_indices = np.argsort(temp_fitness)[-num_to_select:]
    return [population[i] for i in selected_indices]


def genetic_algorithm(problem, num_generations=100, population_size=10, crossover_rate=0.8, mutation_rate=0.2):
    population = initialize_population(problem.box_list, population_size)
    num_individuals = len(population)

    for generation in range(num_generations):
        temp_fitness = [evaluate_fitness(individual, problem) for individual in population]
        print(f"代系：{generation}, 当前适应率：{temp_fitness[0]}")

        # 选择
        selected_individuals = select(population, temp_fitness, num_to_select=population_size // 2)
        ...
        for i in range(len(new_population)):
            if random.random() < mutation_rate:
                new_population[i] = mutate(new_population[i], [box.type for box in problem.box_list])

        population = new_population

    # 在最终群体中找到最佳解决方案
    final_fitness = [evaluate_fitness(individual, problem) for individual in population]
    best_individual = population[np.argmax(final_fitness)]
    return best_individual


def fitness(pack_state, problem):
    # 适应度函数，计算当前状态的利用率
    return pack_state.volume / (problem.container.lx * problem.container.ly * problem.container.lz)


# 遗传算法主逻辑
def basic_heuristic(search_params, problem: Problem, method="genetic"):
    if method == "genetic":
        best_solution = genetic_algorithm(problem)
        block_table = gen_complex_block(problem.container, best_solution, problem.num_list)
        pack_state = PackingState(plan_list=[], avail_list=problem.num_list)
        pack_state.space_stack.push(problem.container)
        while pack_state.space_stack.size() > 0:
            space = pack_state.space_stack.top()
        ...
        print("利用率：{}".format(fitness(pack_state, problem)))
...
def run_solution(box_list: list, container: list):
    ...
    search_params = dict()
    basic_heuristic(search_params, problem, method="genetic")


if __name__ == "__main__":
    CONTAINER = [0, 0, 0, 587, 233, 220]
    BOX_LIST = [[108, 76, 30, 40, 35], [110, 43, 25, 33, 35], [92, 81, 55, 39, 30]]
