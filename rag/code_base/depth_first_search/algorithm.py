...
# 补全放置方案
def complete(pack_state: PackingState, block_table):
    # 不对当前的放置状态进行修改
    tmp = copy.deepcopy(pack_state)
    ...
    # 补全后的使用体积
    pack_state.volume_complete = tmp.volume


# 带深度限制的深度优先搜索算法
def depth_first_search(pack_state: PackingState, depth, branch, block_table):
    global best_pack_state
    if depth != 0:
        ...
        if len(block_list) > 0:
            ...
            for i in range(min(branch, len(block_list))):
                # 放置块
                place = place_block(pack_state, block_list[i])
                # 放置下一个块
                depth_first_search(pack_state, depth - 1, branch, block_table)
                # 移除刚才添加的块
                remove_block(pack_state, block_list[i], place, space)
        else:
            # 转移空间
            old_target = transfer_space(space, pack_state.space_stack)
            if old_target:
                # 放置下一个块
                depth_first_search(pack_state, depth, branch, block_table)
                # 还原转移空间
                transfer_space_back(space, pack_state.space_stack, old_target)
    else:
        # 补全该方案
        complete(pack_state, block_table)
        # 更新最优解
        if pack_state.volume_complete > best_pack_state.volume_complete:
            best_pack_state = copy.deepcopy(pack_state)
...
def estimate(pack_state: PackingState, block_table, search_params):
    ...
    # 空的放置方案
    global best_pack_state
    # tmp_best_pack_state = PackingState()
    best_pack_state = PackingState([], Stack(), [])
    # 开始深度优先搜索
    depth_first_search(pack_state, DEPTH, BRANCH, block_table)
    return best_pack_state.volume_complete
